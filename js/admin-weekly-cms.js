(function () {
  ...
  function preview(item, heading) {
    ...
    if (!item.driveUrl) { setState('Add a Google Drive image link.'); wrap.append(frame); return wrap; }
    const imageUrl = normalizeDriveUrl(item.driveUrl);
    if (!imageUrl) { setState('Invalid Google Drive link.', 'text-red'); wrap.append(frame); return wrap; }

    const loader = node('div', 'absolute inset-0 z-10 flex items-center justify-center bg-surface-warm');
    const loaderImg = document.createElement('img');
    loaderImg.src = APERTURE_ILLUSTRATION;
    loaderImg.alt = 'Aperture';
    loaderImg.width = 180;
    loaderImg.height = 180;
    loaderImg.loading = 'eager';
    ...
    image.addEventListener('error', () => setState('Image could not be loaded. Set Drive sharing to Anyone with the link → Viewer.', 'text-red'), { once: true });
  ...
  function boot() {
    ...
    const observer = new MutationObserver(() => {
      if (document.getElementById(SECTION_ID)) render();
    });
