(function(){
  ...
  const thumb=v=>{const id=driveId(v);return id?`https://drive.google.com/thumbnail?id=${encodeURIComponent(id)}&sz=w2000`:''};
  async function api(mode,options={}){const r=await fetch(API+mode,{cache:'no-store',...options,headers:{...(options.headers||{})}});const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.error||'Request failed.');return b}
  function render(){
    const root=document.querySelector('main'); const anchor=root?.querySelector('#saveAll'); if(!root||!anchor)return;
    ...
    if(!section){
      ...
      const save=el('button','btn-primary text-sm',saving?'Saving…':'Save Weekly Highlights');save.type='button';save.dataset.weeklySave='1';save.onclick=save;
    ...
    const list=section.querySelector('[data-weekly-list]'); if(!list)return;
    list.replaceChildren(...current.items.map((item,index)=>{
      ...
      if(item.driveUrl&&item.imageUrl){const img=document.createElement('img');img.src=item.imageUrl;img.alt=item.title||`Highlight ${index+1}`;img.className='w-full h-full object-cover';img.loading='eager';preview.append(img)}else preview.append(el('div','w-full h-full flex items-center justify-center text-sm text-muted','Add a valid Drive image link'));
      card.append(preview);
  ...
  function boot(){if(document.body.dataset.weeklyFixBooted)return;document.body.dataset.weeklyFixBooted='1';const obs=new MutationObserver(()=>{if(document.querySelector('#saveAll'))render()});obs.observe(document.body,{childList:true,subtree:true});load()}
