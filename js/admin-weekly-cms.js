(function () {
  ...
  function preview(item, heading) {
    ...
    if (!item.driveUrl) {
      setState(REICON_EMPTY, 'Add a Google Drive image link.');
      wrap.append(frame);
      return wrap;
    }
    ...
    if (!imageUrl) {
      setState(REICON_ERROR, 'Invalid Google Drive link.');
      wrap.append(frame);
      return wrap;
    }
  ...
  function markDirty() {
    dirty = true;
    document.querySelector('[data-weekly-admin-save]')?.replaceChildren(document.createTextNode('Save Weekly Highlights • Unsaved'));
  ...
  function renderItem(item, index) {
    ...
    card.append(field('Google Drive image link', item.driveUrl, 'url', 'Google Drive → Share → Anyone with the link → Viewer.', (value) => {
    ...
    const status = node('span', 'text-xs text-muted', item.published ? 'Visible on public page' : 'Draft');
    row.append(published, status);
    details.append(row);
    card.append(details);
    return card;
  }
  ...
  function render() {
    ...
    if (!section) {
      ...
      const save = node('button', 'btn-primary text-sm', 'Save Weekly Highlights');
      save.type = 'button';
      save.dataset.weeklyAdminSave = 'true';
      save.addEventListener('click', saveAll);
      header.append(info, save);
      section.append(header);

      const sharing = node('div', 'mt-4 rounded-xl border border-gold-muted bg-gold-light p-3 text-sm text-sec');
      sharing.append(node('strong', 'text-heading', 'Drive sharing: '), document.createTextNode('set each image to “Anyone with the link” → Viewer. No arbitrary image hosts are accepted.'));
      ...
      add.addEventListener('click', () => {
        if (current.items.length >= MAX_ITEMS) return toast(`Maximum ${MAX_ITEMS} weekly images reached.`, true);
        current.items.push({ ...newItem(), order: current.items.length });
        markDirty();
        render();
      });
      section.append(add);
      ...
      grid.dataset.weeklyItems = 'true';
      section.append(grid);

      const empty = node('div', 'mt-5 rounded-xl border border-theme p-6 text-center text-sm text-muted');
      empty.dataset.weeklyEmpty = 'true';
    ...
    const grid = section.querySelector('[data-weekly-items]');
    const empty = section.querySelector('[data-weekly-empty]');
    if (grid) {
      grid.replaceChildren(...current.items.map((item, index) => renderItem(item, index)));
      grid.classList.toggle('hidden', current.items.length === 0);
    }
    if (empty) {
      empty.textContent = current.items.length ? `${current.items.length} image${current.items.length === 1 ? '' : 's'} configured.` : 'No weekly images yet. Click “＋ Add Image” to create the first one.';
  ...
  async function saveAll() {
    ...
    try {
      ...
      current.items.forEach((item, index) => {
        item.order = index;
        item.driveUrl = String(item.driveUrl || '').trim();
        item.imageUrl = normalizeDriveUrl(item.driveUrl) || '';
        if (item.driveUrl && !item.imageUrl) throw new Error(`Highlight ${index + 1} has an invalid Google Drive URL.`);
        if (item.published && !item.imageUrl) throw new Error(`Highlight ${index + 1} must have a valid Drive image before publishing.`);
      });
  ...
  async function load() {
    try {
      ...
      current = {
        items: items.slice(0, MAX_ITEMS).map((item, index) => ({ ...newItem(), ...item, order: Number.isInteger(item?.order) ? item.order : index }))
      };
    } catch (error) {
