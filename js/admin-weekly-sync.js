(function () {
  ...
  function markDirty(value = true) {
    ...
    if (saveAll && dirty) saveAll.textContent = 'Save All Changes • Unsaved';
  }
  ...
  function makeCard(slot, item) {
    ...
    card.append(field('Image HTTPS URL', item, 'imageUrl', 'url', (value) => { item.imageUrl = value.trim(); markDirty(); refreshPreview(); }), preview);
  ...
  function renderSection() {
    ...
    const saveAll = document.querySelector('#saveAll');
    if (saveAll && !saveAll.dataset.weeklySyncAttached) {
      saveAll.dataset.weeklySyncAttached = 'true';
      saveAll.addEventListener('click', () => { if (dirty) window.setTimeout(save, 0); });
    }
  }
