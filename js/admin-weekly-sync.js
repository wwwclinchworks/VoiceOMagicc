(function () {
  ...
  function makeCard(slot, item) {
    ...
    const refresh = () => {
      ...
      if (!item.imageUrl) { preview.append(node('p', 'p-5 text-sm text-muted text-center', 'Image preview will appear here.')); return; }
      if (!validHttps(item.imageUrl)) { preview.append(node('p', 'p-5 text-sm text-red text-center', 'Enter a valid HTTPS image URL to preview it.')); return; }
      const img = document.createElement('img');
      img.src = item.imageUrl; img.alt = item.title || 'Weekly highlight preview'; img.loading = 'lazy'; img.decoding = 'async'; img.className = 'w-full aspect-[4/3] object-cover';
    ...
    card.append(field('Image HTTPS URL', item, 'imageUrl', 'url', (value) => { item.imageUrl = value.trim(); markDirty(); refresh(); }), preview);
    const fields = node('div', 'mt-4 space-y-4');
    fields.append(field('Title (optional)', item, 'title', 'text', (value) => { item.title = value; markDirty(); }), field('Description (optional)', item, 'description', 'textarea', (value) => { item.description = value; markDirty(); }));
    card.append(fields);
    const publish = node('label', 'inline-flex items-center gap-2 text-sm font-semibold text-sec cursor-pointer mt-4');
    const checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.checked = item.published === true;
    checkbox.addEventListener('change', () => { item.published = checkbox.checked; markDirty(); });
    publish.append(checkbox, node('span', '', 'Published')); card.append(publish);
    return card;
  }
  ...
  function renderSection() {
    ...
    if (!section) {
      ...
      info.append(node('h2', 'text-xl md:text-2xl font-bold text-heading', '📸 Weekly Highlights'), node('p', 'text-sm font-medium text-gold mt-1', 'Where this changes: resources.html → Weekly Highlights'), node('p', 'text-xs text-muted mt-2', 'Manage exactly two weekly images. Published highlights are shown publicly.'));
      const saveButton = node('button', 'btn-primary text-sm', dirty ? 'Save Weekly Highlights • Unsaved' : 'Save Weekly Highlights');
      saveButton.type = 'button'; saveButton.dataset.weeklySyncSave = 'true'; saveButton.addEventListener('click', save);
      header.append(info, saveButton); section.append(header);
      const grid = node('div', 'grid grid-cols-1 lg:grid-cols-2 gap-5 mt-5'); grid.dataset.weeklySyncGrid = 'true'; section.append(grid);
      const children = Array.from(dashboard.children);
      const anchor = children.find((child) => child.querySelector?.('h2')?.textContent?.includes('Featured Video')) || children.find((child) => child.querySelector?.('h2')?.textContent?.includes('Resources'));
      if (anchor) dashboard.insertBefore(section, anchor); else dashboard.insertBefore(section, dashboard.children[1] || null);
    }
  ...
  async function save() {
    ...
    saving = true;
    const button = document.querySelector('[data-weekly-sync-save]'); if (button) { button.disabled = true; button.textContent = 'Saving…'; }
    try {
      ...
      const response = await fetch(ENDPOINT, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ weeklyHighlights: current }), cache: 'no-store' });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || 'Unable to save Weekly Highlights.');
      current = clone(result.weeklyHighlights || current); dirty = false; renderSection(); toast('Weekly Highlights saved successfully.');
  ...
  async function load() {
    try {
      ...
      current = clone({ highlight1: { imageUrl: '', title: '', description: '', published: false }, highlight2: { imageUrl: '', title: '', description: '', published: false } });
      toast(error.message || 'Unable to load Weekly Highlights.', true);
    }
    loaded = true; renderSection();
  }

  function boot() {
    if (observer) return;
    observer = new MutationObserver(() => renderSection());
