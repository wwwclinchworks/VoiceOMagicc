(function () {
  ...
  function node(tag, classes, value) {
  ...
  function inputField(label, item, key, type, onChange) {
  ...
  function preview(item) {
    const box = node('div', 'mt-3 rounded-xl border border-theme overflow-hidden bg-surface-warm min-h-[160px] flex items-center justify-center');
    if (validHttps(item.imageUrl)) {
      const image = document.createElement('img');
      image.src = item.imageUrl;
      image.alt = item.title || 'Weekly highlight preview';
      image.loading = 'lazy';
      image.decoding = 'async';
      image.className = 'w-full aspect-[4/3] object-cover';
      image.addEventListener('error', () => {
        box.replaceChildren(node('p', 'p-5 text-sm text-red text-center', 'Unable to load this image URL. Check the HTTPS URL.'));
      }, { once: true });
      box.append(image);
    } else {
      box.append(node('p', 'p-5 text-sm text-muted text-center', item.imageUrl ? 'Enter a valid HTTPS image URL to preview it.' : 'Image preview will appear here.'));
    }
    return box;
  }
  ...
  function showToast(message, bad) {
    const toast = node('div', 'fixed right-5 bottom-5 z-[140] max-w-sm rounded-xl px-4 py-3 text-sm font-semibold text-white shadow-xl', message);
    toast.style.background = bad ? '#b42318' : '#111827';
    document.body.append(toast);
    window.setTimeout(() => toast.remove(), 3200);
  }

  function rerender() {
    const section = document.querySelector('[data-weekly-highlights-admin]');
    if (!section || !current) return;
    const grid = section.querySelector('[data-weekly-grid]');
    if (!grid) return;
  ...
  async function save() {
    ...
    try {
      ...
      showToast(error.message || 'Unable to save Weekly Highlights.', true);
    } finally {
      if (button) { button.disabled = false; button.textContent = 'Save Weekly Highlights'; }
    }
  }
  ...
  function render() {
    ...
    const section = node('section', 'material-card p-5');
    section.dataset.weeklyHighlightsAdmin = 'true';
    ...
    saveButton.dataset.weeklySave = 'true';
    saveButton.addEventListener('click', save);
    header.append(info, saveButton);
    section.append(header);
    const grid = node('div', 'grid grid-cols-1 lg:grid-cols-2 gap-5 mt-5');
    grid.dataset.weeklyGrid = 'true';
    section.append(grid);

    const existing = Array.from(dashboard.children);
    const anchor = existing.find((child) => child.querySelector?.('h2')?.textContent?.includes('Featured Video')) || existing.find((child) => child.querySelector?.('h2')?.textContent?.includes('Resources'));
  ...
  function boot() {
    if (document.querySelector('[data-weekly-highlights-admin]')) return;
