...
const config=JSON.parse(fs.readFileSync(path.join(root,'vercel.json'),'utf8'));const headerText=JSON.stringify(config.headers||[]);for(const h of ['Content-Security-Policy','Strict-Transport-Security','X-Content-Type-Options','X-Frame-Options','Referrer-Policy','Permissions-Policy','X-DNS-Prefetch-Control'])if(!headerText.includes(h))fail(`Missing security header: ${h}`);if(!headerText.includes('max-age=31536000'))fail('HSTS max-age is below one year.');
