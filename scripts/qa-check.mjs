...
const config=JSON.parse(fs.readFileSync(path.join(root,'vercel.json'),'utf8'));const headerText=JSON.stringify(config.headers||[]);for(const h of ['X-Content-Type-Options','X-Frame-Options','Referrer-Policy','Permissions-Policy'])if(!headerText.includes(h))fail(`Missing security header: ${h}`);
