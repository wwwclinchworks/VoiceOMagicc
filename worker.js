...
const SESSION_COOKIE = 'vom_admin';
...
function normalizeWeeklyItem(value) {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const drive = clean(source.driveUrl, 2000);
  const imageUrl = driveImageUrl(drive) || (source.imageUrl && httpsUrl(source.imageUrl) ? clean(source.imageUrl, 2000) : '');
  if (drive && !imageUrl) throw Object.assign(new Error('Weekly highlight Drive URL must be a valid Google Drive image sharing URL.'), { status: 400 });
  if (source.published === true && !imageUrl) throw Object.assign(new Error('Every published weekly highlight needs a valid image URL.'), { status: 400 });
  return {
    id: uid(source.id),
    driveUrl: drive,
    imageUrl,
...
function normalizeHighlights(value) {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const raw = Array.isArray(source.items) ? source.items : [source.highlight1, source.highlight2].filter(Boolean);
  return { items: raw.map(normalizeWeeklyItem).sort((a, b) => a.order - b.order) };
}
...
function publicSnapshot(cms) {
  ...
  c.weeklyHighlights.items = c.weeklyHighlights.items.filter((item) => item.published && item.imageUrl);
  return c;
}
